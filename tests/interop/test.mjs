...
const outputGo = join(TESTS_DIR, 'output_dart');
if (existsSync(outputGo)) rmSync(outputGo, { recursive: true });
mkdirSync(join(outputGo, 'scalars'), { recursive: true });

run(`cd ${TESTS_DIR} && podman build ${COMMON} -t specodec-interop-dart -f Containerfile \
...
const container = execSync(`podman create specodec-interop-dart /bin/true`).toString().trim();
run(`podman cp ${container}:/app/output_dart/. ${outputGo}/`);
...
run(`cd ${LANG_ROOT} && npx tsp compile ${CACHE}/alltypes.tsp --emit=@specodec/typespec-emitter-dart \
  --option @specodec/typespec-emitter-dart.emitter-output-dir=${emitGen}`);

run(`cd ${TESTS_DIR} && podman build ${COMMON} -t specodec-emit-dart -f Containerfile.emit \
  --build-context specodec-go=${LANG_ROOT} \
...
console.log('\n=== Step 5: Emit roundtrip test ===');
const outputEmitGo = join(TESTS_DIR, 'output_emit_dart');
if (existsSync(outputEmitGo)) rmSync(outputEmitGo, { recursive: true });
mkdirSync(outputEmitGo, { recursive: true });

run(`cd ${TESTS_DIR} && podman build ${COMMON} -t specodec-emit-run-dart -f Containerfile.emit-run \
  --build-context specodec-go=${LANG_ROOT} \
  --build-context emit=${TESTS_DIR}/emit \
  --build-context emit_gen=${emitGen} \
  --build-context vectors=${vectorsDir} .`);

const container2 = execSync(`podman create specodec-emit-run-dart /bin/true`).toString().trim();
run(`podman cp ${container2}:/app/output_emit_dart/. ${outputEmitGo}/`);
